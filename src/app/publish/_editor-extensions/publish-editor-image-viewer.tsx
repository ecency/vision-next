            ...
            <div
              className={clsx(
                "absolute top-0 left-0 w-full h-full items-center justify-center",
                  typeof src === "string" && src.startsWith("blob") ? "flex" : "hidden"
              )}
            >
              <UilSpinner className="w-12 h-12 text-white animate-spin" />
            </div>
            <Image
              className={clsx("w-auto max-w-full", typeof src === "string" && src.startsWith("blob") && "grayscale blur-sm")}
              src={src}
              width={200}
              height={200}
              alt={alt}
      ...
      >
        <PopoverContent>
          <div className="flex gap-2">
            {typeof src === "string" && src.includes("https://images.ecency.com") && (
              <Button
                noPadding={true}
                size="xs"
