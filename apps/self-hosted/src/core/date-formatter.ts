...
import { enUS, es, de, fr, pt, ru, ko, ja, zhCN, it, tr, pl, uk, vi, id } from "date-fns/locale";
 ...
 */
export function formatCustom(date: Date | string | number, formatStr: string): string {
  const dateObj = typeof date === "string" || typeof date === "number" ? new Date(date) : date;
  return format(dateObj, convertFormatPattern(formatStr), { locale: getLocale() });
}
