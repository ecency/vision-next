...
export function EntryPageStatsByReferrers({ totalViews, cleanedPathname, dateRange }: Props) {
  ...
  return referrersList.length > 0 ? (
    <div className="flex flex-col w-full gap-2 text-sm">
      ...
      {referrersList.map(([referrer, views]) => (
        <div
          className="animate-fade-in-up rounded-md overflow-hidden relative w-full flex items-center justify-between bg-gray-100 dark:bg-gray-900"
          key={referrer}
        >
          <div
            style={{ width: `${totalViews > 0 ? (views * 100) / totalViews : 0}%` }}
