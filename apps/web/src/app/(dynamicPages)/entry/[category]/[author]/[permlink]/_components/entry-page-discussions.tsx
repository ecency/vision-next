...
function isCommunity(x: unknown): x is Community {
...
export function EntryPageDiscussions({ entry: initialEntry, category }: Props) {
  ...
  const { data: communityRaw } = getCommunityCache(category).useClientQuery();
  ...
  const community = isCommunity(communityRaw) ? communityRaw : undefined;
  ...
  if (!entry || !community) return null;
