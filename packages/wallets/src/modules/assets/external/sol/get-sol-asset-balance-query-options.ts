...
export function getSolAssetBalanceQueryOptions(address: string) {
  return queryOptions({
    ...
    queryFn: async () => {
      const baseUrl = `${ConfigManager.getValidatedBaseUrl()}/private-api/balance/sol/${encodeURIComponent(
        address
      )}`;
      ...
      if (primaryResponse?.ok) {
        return +parsePrivateApiBalance(await primaryResponse.json(), "sol").balanceString;
      }
      ...
      return +parsePrivateApiBalance(await fallbackResponse.json(), "sol").balanceString;
    },
  });
}
