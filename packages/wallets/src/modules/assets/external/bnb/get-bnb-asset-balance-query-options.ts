...
export function getBnbAssetBalanceQueryOptions(address: string) {
  return queryOptions({
    ...
    queryFn: async () => {
      const baseUrl = `${ConfigManager.getValidatedBaseUrl()}/private-api/balance/bnb/${encodeURIComponent(
        address
      )}`;
      ...
      if (primaryResponse?.ok) {
        return +parsePrivateApiBalance(await primaryResponse.json(), "bnb").balanceString;
      }
      ...
      return +parsePrivateApiBalance(await fallbackResponse.json(), "bnb").balanceString;
    },
  });
}
